...
# Modifications Copyright 2023 Amazon.com, Inc. or its affiliates. Licensed under the Apache License, version 2.
#
...
# This file is edited by us for our special pre-processing and metrics computation.
# This file is edited to contain our fixed version of tokenization.
"""
Fine-tuning generative models using consistent tokenization
"""
...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        if 'train' in raw_datasets:
            # Remove the instances that are not extractive
            ...
            if len(answer["text"]) > 0:
                ...
                if answer['text'][0].lower() in contexts[ans_idx].lower() and inputs[ans_idx][curr_target_idx: curr_target_idx + len(answer["text"][0])+1] != answer["text"][0]:
                    if re.search(f"\W{re.escape(answer['text'][0].lower())}\W", contexts[ans_idx].lower()):
                        ...
                        curr_target_idx = re.search(f"\W{re.escape(answer['text'][0].lower())}\W", contexts[ans_idx].lower()).span()[0] + 1
                    elif re.search(f"\W{re.escape(answer['text'][0].lower())}$", contexts[ans_idx].lower()):
                        curr_target_idx = re.search(f"\W{re.escape(answer['text'][0].lower())}$", contexts[ans_idx].lower()).span()[0] + 1
            else:
                curr_target_idx = 0
        ...
        for instance_idx in range(len(model_inputs['input_ids'])):
            ...
            if 't5' in model_args.model_name_or_path:
                if str(labels['input_ids'][instance_idx][:labels['input_ids'][instance_idx].index(1)])[1:-1] not in str(model_inputs['input_ids'][instance_idx]):
                    ...
                    end_idx = min(end_idx, start_idx + data_args.max_answer_length - 2)
                ...
                end_idx = min(end_idx, start_idx + data_args.max_answer_length - 3)
